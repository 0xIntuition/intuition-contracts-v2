...
const ROLE_NAMES: Record<string, string> = {
	...
	'0x643676d2b1119408842d7edf610075af886c8b1197b1548d80145808930e1697': 'ADMIN_ROLE', // METAERC20ADMIN_ROLE
	'0xc9e19f1275fdf2eae30e12c0733c54aa5c1802256d8c46fe9ba9f631bdd6f0b5': 'VALIDATOR_ROLE', // METAERC20VALIDATOR_ROLE
};
...
function normalizeRoleNames(): Record<string, string> {
	return Object.fromEntries(
		Object.entries(ROLE_NAMES).map(([roleHash, roleName]) => [roleHash.toLowerCase(), roleName])
	);
}
...
async function fetchEtherscanLogsByTopic(
	...
	while (true) {
		...
		if (!Array.isArray(payload.result)) {
			const errorText =
				typeof payload.result === 'string' ? payload.result : payload.message || 'unknown';
		...
		const pageLogs = payload.result.map((rawLog) =>
			mapEtherscanLogToProviderLog(rawLog as Record<string, unknown>)
		);
	...
	while (rangeStartBlock <= toBlock) {
		...
		try {
			...
			if (suggestedRangeLimit <= 50) {
				...
				return fetchRoleLogsFromEtherscan(
					contractAddress,
					chainId,
					fromBlock,
					toBlock,
			...
			console.log(
				`[${contractAddress}] reducing block chunk size to ${rangeChunkSize} due to provider limits`
			);
		}
	}
...
function buildSummaryRows(
	...
	return Object.entries(roleNamesByHash)
		...
		.sort((leftRow, rightRow) => {
			if (rightRow.holderCount !== leftRow.holderCount)
				return rightRow.holderCount - leftRow.holderCount;
			return leftRow.role.localeCompare(rightRow.role);
		});
}

async function main() {
	const { contractAddress, chainId, fromBlock, toBlock, rpcUrl, etherscanApiKey } =
		parseCliArguments();
	...
	if ((resolvedToBlock as number) < fromBlock) {
		throw new Error(
			`Invalid block range: fromBlock ${fromBlock} is greater than toBlock ${resolvedToBlock}`
		);
	}
