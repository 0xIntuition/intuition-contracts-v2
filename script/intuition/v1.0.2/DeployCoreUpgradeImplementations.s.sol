...
contract DeployCoreUpgradeImplementations is SetupScript {
    ...
    function _writeOperationsArtifact() internal {
        uint256 minDelay = TimelockController(payable(UPGRADES_TIMELOCK)).getMinDelay();
        TimelockOperationArtifact[] memory operations = new TimelockOperationArtifact[](4);
        ...
        operations[1] = TimelockOperationArtifact({
            label: "OffsetProgressiveCurve upgrade",
            target: OFFSET_CURVE_PROXY_ADMIN,
            value: ZERO_VALUE,
            data: abi.encodeCall(
                ProxyAdmin.upgradeAndCall,
                (ITransparentUpgradeableProxy(OFFSET_CURVE_PROXY), address(offsetProgressiveCurveImplementation), "")
        ...
        operations[2] = TimelockOperationArtifact({
            label: "SatelliteEmissionsController upgrade",
            target: SATELLITE_EC_PROXY_ADMIN,
            value: ZERO_VALUE,
            data: abi.encodeCall(
                ProxyAdmin.upgradeAndCall,
                (
        ...
        operations[3] = TimelockOperationArtifact({
            label: "AtomWallet beacon upgrade",
            target: ATOM_WALLET_BEACON,
            value: ZERO_VALUE,
            data: abi.encodeCall(UpgradeableBeacon.upgradeTo, (address(atomWalletImpl))),
            predecessor: ZERO_PREDECESSOR,
            salt: ZERO_SALT,
            delay: minDelay
        });

        _writeTimelockOperationsArtifact(OPERATIONS_ARTIFACT_PATH, UPGRADES_TIMELOCK, operations);
    }
}
