...
import {IMultiVaultConfig, GeneralConfig, AtomConfig, TripleConfig, WalletConfig, VaultFees, BondingCurveConfig, WrapperConfig} from "src/interfaces/IMultiVaultConfig.sol";
...
contract DeployV2ToTestnet is Script {
    ...
    address public constant entryPoint =
        0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789; // EntryPoint on Base
    ...
    bytes32 public constant merkleRoot =
        0x139108d0708628265813796f3ee047525c413d5c29953de8f2fe01c0f1cca35d; // precomputed Merkle root for testing the airdrop claim
    ...
    function run() external {
        ...
        // Allow the script to run only on Local Geth to prevent accidental deployments on mainnet
        ...
        console.log(
            "AtomWalletFactory proxy address: ",
            address(atomWalletFactoryProxy)
        );

        // deploy WrappedERC20 implementation contract
        wrappedERC20 = new WrappedERC20();
        console.log(
            "WrappedERC20 implementation address: ",
            address(wrappedERC20)
        );

        // deploy WrappedERC20Beacon pointing to the WrappedERC20 implementation contract
        wrappedERC20Beacon = new UpgradeableBeacon(
            address(wrappedERC20),
            admin
        );
        console.log(
            "WrappedERC20Beacon address: ",
            address(wrappedERC20Beacon)
        );

        // deploy the WrappedERC20Factory contract
        wrappedERC20Factory = new WrappedERC20Factory();
        TransparentUpgradeableProxy wrappedERC20FactoryProxy = new TransparentUpgradeableProxy(
                address(wrappedERC20Factory),
                admin,
                ""
            );
        wrappedERC20Factory = WrappedERC20Factory(
            address(wrappedERC20FactoryProxy)
        );
        console.log(
            "WrappedERC20Factory proxy address: ",
            address(wrappedERC20FactoryProxy)
        );
        ...
        console.log(
            "OffsetProgressiveCurve address: ",
        ...
        console.log(
            "TrustBonding implementation address: ",
        ...
        atomConfig = AtomConfig({
            atomCreationProtocolFee: 0.01 * 1e18,
        ...
        console.log(
            "MultiVaultConfig implementation address: ",
            address(multiVaultConfig)
        );
        TransparentUpgradeableProxy multiVaultConfigProxy = new TransparentUpgradeableProxy(
                address(multiVaultConfig),
                admin,
                ""
            );
        ...
        console.log(
            "MultiVaultConfig proxy address: ",
            address(multiVaultConfigProxy)
        );
        ...
        console.log(
            "TrustVestedMerkleDistributor implementation address: ",
            address(trustVestedMerkleDistributor)
        );

        // deploy the TrustVestedMerkleDistributor proxy contract
        TransparentUpgradeableProxy trustVestedMerkleDistributorProxy = new TransparentUpgradeableProxy(
                address(trustVestedMerkleDistributor),
                admin,
                ""
            );

        trustVestedMerkleDistributor = TrustVestedMerkleDistributor(
            address(trustVestedMerkleDistributorProxy)
        );
        console.log(
            "TrustVestedMerkleDistributor proxy address: ",
            address(trustVestedMerkleDistributorProxy)
        );

        // initialize the TrustVestedMerkleDistributor contract
        TrustVestedMerkleDistributor.VestingParams
            memory distributorVestingParams = TrustVestedMerkleDistributor
                .VestingParams({
                    owner: admin,
                    trust: address(trustToken),
                    trustBonding: address(trustBonding),
