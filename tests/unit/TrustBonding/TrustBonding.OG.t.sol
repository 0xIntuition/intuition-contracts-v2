...
//     ProxyAdmin public proxyAdmin = ProxyAdmin(0x4f81992FCe2E1846dD528eC0102e6eE1f61ed3e2); // pre-calculated proxy
// admin address
...
//         vm.prank(admin);
//         trustBonding.initialize(address(multiVault), systemUtilizationLowerBound, personalUtilizationLowerBound);
...
//         uint256 currentEpochEndTimestamp = trustBonding.epochTimestampEnd(currentEpoch);
...
//         bytes32 outerSlot = keccak256(abi.encode(user, uint256(14))); // 14 = storage slot of
// userClaimedRewardsForEpoch mapping
...
//         uint256 invalidPersonalUtilizationLowerBound = trustBonding.BASIS_POINTS_DIVISOR() + 1; // 100% + 1 basis
// point
...
//         currentEpochEndTimestamp = trustBonding.epochTimestampEnd(currentEpoch);
...
//         uint256 totalClaimedRewardsForEpoch = trustBonding.totalClaimedRewardsForEpoch(trustBonding.currentEpoch() -
// 1);
...
//         uint256 aliceBondedBalance = trustBonding.userBondedBalanceAtEpochEnd(alice, trustBonding.currentEpoch() -
// 1);
//         uint256 totalBondedBalance = trustBonding.totalBondedBalanceAtEpochEnd(trustBonding.currentEpoch() - 1);
//         uint256 aliceShareBasisPoints = (aliceBondedBalance * trustBonding.BASIS_POINTS_DIVISOR()) /
// totalBondedBalance;
...
//         uint256 bobExpectedRewards = bobRawRewards * personalUtilizationLowerBound /
...
//         // advance into the 4th epoch to be able to claim rewards when the utilizationRatio is not guaranteed to be
// 100%
//         _advanceEpochs(4);
//     }
...
//     function
// test_getSystemUtilizationRatio_shouldReturnMaxRatioIfUtilizationIncreasedButNoRewardsWereClaimedPreviously(
//     ) external {
//         uint256 previousEpoch = trustBonding.currentEpoch() - 1;
//         multiVault.setTotalUtilizationForEpoch(previousEpoch - 1, 1000);
...
//             systemUtilizationLowerBound + (trustBonding.BASIS_POINTS_DIVISOR() - systemUtilizationLowerBound) / 2; //
// 25 % + half of remaining 75 % = 6250 bps
...
//     function
// test_getPersonalUtilizationRatio_shouldReturnMaxRatioIfUtilizationIncreasedButNoRewardsWereClaimedPreviously(
//     ) external {
//         uint256 previousEpoch = trustBonding.currentEpoch() - 1;
//         multiVault.setUserUtilizationForEpoch(alice, previousEpoch - 1, 1000);
...
//         uint256 previousEpoch = trustBonding.currentEpoch() - 1;
//         multiVault.setUserUtilizationForEpoch(alice, previousEpoch - 1, 1000);
//         multiVault.setUserUtilizationForEpoch(alice, previousEpoch, 1500); // utilization increased by 50% of the
// target
...
//             personalUtilizationLowerBound + (trustBonding.BASIS_POINTS_DIVISOR() - personalUtilizationLowerBound) /
// 2; // 6250 bps
