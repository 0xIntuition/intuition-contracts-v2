...
contract TrustBonding is ITrustBonding, AccessControlUpgradeable, VotingEscrow {
    ...
    /// @notice The system utilization lower bound in basis points (represents the minimum possible system utilization ratio)
    uint256 public systemUtilizationLowerBound;

    /// @notice The personal utilization lower bound in basis points (represents the minimum possible personal utilization ratio)
    ...
    function initialize(
        ...
        if (_systemUtilizationLowerBound > BASIS_POINTS_DIVISOR) {
            revert Errors.TrustBonding_InvalidUtilizationLowerBound();
        }

        if (_personalUtilizationLowerBound > BASIS_POINTS_DIVISOR) {
            revert Errors.TrustBonding_InvalidUtilizationLowerBound();
        }
    ...
    /**
     * @notice Returns the percentage of TRUST tokens locked in the contract out of the total supply
    ...
    function lockedTrustPercentage() public view returns (uint256) {
        ...
        return totalLocked() * BASIS_POINTS_DIVISOR / trustSupply;
    }
    ...
    function totalBondedBalanceAtEpochEnd(uint256 _epoch) public view returns (uint256) {
        if (_epoch > currentEpoch()) {
            revert Errors.TrustBonding_InvalidEpoch();
        }
    ...
    /**
     * @notice Calculates the amount of TRUST tokens to be emitted per epoch, based on bonding percentage and max emission
    ...
    function trustPerEpoch(uint256 _epoch) public view returns (uint256) {
        ...
        uint256 lockedTrustPct = lockedTrustPercentage();
    ...
    function getSystemUtilizationRatio(uint256 _epoch) public view returns (uint256) {
        ...
        // If there was no target utilization in the previous epoch, any increase in utilization is rewarded with the max ratio.
        ...
        // Normalize the final utilizationRatio to be within the bounds of the systemUtilizationLowerBound and BASIS_POINTS_DIVISOR
    ...
    function getPersonalUtilizationRatio(address _account, uint256 _epoch) public view returns (uint256) {
        ...
        // Normalize the final utilizationRatio to be within the bounds of the personalUtilizationLowerBound and BASIS_POINTS_DIVISOR
        return
     ...
     * @dev Rewards for epoch `n` are claimable in epoch `n + 1`. If the user forgets to claim their rewards for epoch `n`,
     *      they are effectively forfeited. Note that the user is free to claim their rewards to any address they choose.
    ...
    function claimRewards(address recipient) external whenNotPaused nonReentrant {
        ...
        if (rawUserRewards == 0) {
            revert Errors.TrustBonding_NoRewardsToClaim();
        }
        ...
        if (multiVault.protocolFeeDistributionEnabledAtEpoch(previousEpoch)) {
            ...
            if (accumulatedProtocolFeesForPreviousEpoch > 0 && maxClaimableProtocolFees == 0) {
                revert Errors.TrustBonding_ProtocolFeesNotSentToTrustBondingYet();
            }
            ...
            if (accumulatedProtocolFeesForPreviousEpoch > 0 && maxClaimableProtocolFees > 0) {
                ...
                if (userProtocolFees > 0) {
                    ...
                    if (userProtocolFees > IERC20(token).balanceOf(address(this))) {
                        revert Errors.TrustBonding_ClaimableProtocolFeesExceedBalance();
                    }
    ...
    function setMaxClaimableProtocolFeesForPreviousEpoch(uint256 _maxClaimableProtocolFees) external {
        ...
        if (msg.sender != address(multiVault)) {
            revert Errors.TrustBonding_OnlyMultiVault();
        }
        ...
        if (maxClaimableProtocolFeesForEpoch[previousEpoch] > 0) {
            revert Errors.TrustBonding_MaxClaimableProtocolFeesAlreadySet();
        }
    ...
    /**
     * @notice Returns the normalized utilization ratio, adjusted for the desired range (lowerBound, BASIS_POINTS_DIVISOR)
