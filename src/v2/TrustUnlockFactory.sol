...
contract TrustUnlockFactory is Ownable2Step, ReentrancyGuard {
     ...
     * @dev Only recipients and unlockAmounts vary for each TrustUnlock contract - the rest of the parameters are the same
    ...
    /**
     * @notice Internal function to create a new TrustUnlock contract for a recipient and fund it with the specified amount
     * @dev Assumes that the TrustUnlockFactory contract is funded with enough TRUST tokens to fund the newly created TrustUnlock contract
     * @param recipient The address of the recipient of the TrustUnlock
     * @param unlockAmount The amount of tokens to be unlocked
     * @param unlockBegin The timestamp when the unlock period begins
     * @param unlockCliff The timestamp when the unlock cliff ends
    ...
    function _createTrustUnlock(
        ...
        // Check that this contract has enough Trust tokens to fund the unlock contract
        uint256 factoryBalance = trustToken.balanceOf(address(this));

        if (factoryBalance < unlockAmount) {
            revert Errors.Unlock_InsufficientBalance(factoryBalance, unlockAmount);
        }

        // Transfer the factory's Trust tokens to the TrustUnlock contract
        trustToken.safeTransfer(trustUnlock, unlockAmount);

        // Emit an event to indicate that the TrustUnlock contract has been funded
        emit TrustUnlockFunded(trustUnlock, unlockAmount);
    }
}
