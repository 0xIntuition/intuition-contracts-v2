...
// src/token/MetaERC20Hub.sol

/// @title MetaERC20Hub
/// @notice Canonical bridge-side contract for locking and dispatching ERC20 tokens
/// @dev Handles UnlockRequest messages to release canonical tokens. Transfers tokens into custody
///      and tracks locked balance. Emits cross-chain messages using MintRequest to paired Spokes.
///      Also buffers SecurityRelay messages for high-value transfers requiring manual approval.
///
/// Hub behavior:
/// - Locks canonical tokens from users via safeTransferFrom
/// - Always sends MintRequest messages to destination Spokes
/// - Receives UnlockRequest messages to release locked tokens
/// - Buffers SecurityRelay messages for VALIDATOR_ROLE approval
/// - Converts between source and Hub token decimal precision
/// - Tracks total locked balance for accounting validation
///
/// Must be paired with MetaERC20Spoke contracts on remote chains.
///
contract MetaERC20Hub is MetaERC20Base {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using TypeCasts for bytes32;
    using TypeCasts for address;
    ...
    bytes32 public constant VALIDATOR_ROLE = keccak256("METAERC20VALIDATOR_ROLE");

    /// @notice Maximum number of transfer IDs allowed in a buffered relay batch query
    uint256 public constant MAX_RELAY_QUERY_BATCH = 500;
    ...
    |        | MetaERC20Base            |        |                 |
    |--------|--------------------------|--------|-----------------|
    | 67     | lockedBalance            | 32 B   | Hub-specific    |
    | 68     | __reservedSlot63         | 12 B   |                 |
    |        | wrappedToken             | 20 B   | Hub-specific    |
    | 69     | relayBuffer              | —      | Mapping anchor  |
    | 70     | relayBufferByIndex       | —      | Mapping anchor  |
    | 71     | relayBufferIndex         | —      | Mapping anchor  |
    | 72     | relayBufferCount         | 32 B   |                 |
    | 73–123 | __gap (Hub)              | 50x32B | Reserved (Hub)  |
    ----------------------------------------------------------------
    Total declared slots: 123
    //////////////////////////////////////////////////////////////*/

    /// @notice Total amount of tokens currently held by the Hub and locked for cross-chain transfers
    /// @dev Used to validate UnlockRequests
    uint256 public lockedBalance;

    /// @dev Reserved padding to align `wrappedToken` at the low bytes of slot 63
    uint96 private __reservedSlot63;

    /// @notice The canonical ERC20 token managed by the Hub
    /// @dev Stored in the low 20 bytes of slot 63
    IERC20Upgradeable public wrappedToken;

    /// @notice Buffer for SecurityRelay messages awaiting approval or further processing
    mapping(bytes32 => MetaERC20MessageStruct) public relayBuffer;

    /// @notice Mapping from buffer index to transfer ID for efficient enumeration
    /// @dev Used with relayBufferIndex to provide O(1) add/remove operations while preserving enumeration capability
    mapping(uint256 => bytes32) public relayBufferByIndex; // index → transferId

    /// @notice Mapping from transfer ID to buffer index for efficient removal
    /// @dev Used to quickly locate a transfer's position in relayBufferByIndex for O(1) removal
    mapping(bytes32 => uint256) public relayBufferIndex; // transferId → index

    /// @notice Total number of transfers currently in the relay buffer
    /// @dev Incremented when transfers are added, decremented when removed. Used as the next available index.
    uint256 public relayBufferCount; // total count

    /*//////////////////////////////////////////////////////////////
                            ERRORS
    //////////////////////////////////////////////////////////////*/

    /// @notice Thrown when attempting to buffer a SecurityRelay that has already been buffered
    error DuplicateRelay();

    /// @notice Thrown when too many transfer IDs are passed to a relay batch query
    error ExceedsRelayQueryBatchLimit();

    /// @notice Thrown when the provided token address is zero or otherwise invalid
    error InvalidTokenAddress();

    /// @notice Thrown when the Hub's locked balance is insufficient to fulfill a withdrawal
    error LockedBalanceUnderflow();

    error InvalidWrappedToken(address wrappedToken);

    error DoaNotContract(address _wrappedToken);

    /*//////////////////////////////////////////////////////////////
                            EVENTS
    //////////////////////////////////////////////////////////////*/

    /// @notice Emitted when an admin manually unlocks and refunds a transfer using EMERGENCY_UNLOCK
    /// @param transferId The ID of the transfer that was force-unlocked
    /// @param adminAddress The address of the admin who triggered the unlock
    /// @param recipientAddress The recipient who received the unlocked funds
    event MetaERC20AdminUnlock(bytes32 indexed transferId, address indexed adminAddress, address recipientAddress);

    /// @notice Emitted once during initialization to capture deployment config
    /// @param localDomain The Metalayer domain ID of this chain
    /// @param router The address of the MetalayerRouter
    /// @param token The address of the wrapped canonical token
    /// @param tokenDecimals Number of decimals used by the wrapped token
    /// @param version The MetaERC20 message protocol version in use
    /// @param ttlWindow The seconds a transfer must persist before pruning allowed
    event MetaERC20HubInitialized(
    ...
    /// @notice Emitted when a SecurityRelay message is buffered for manual approval
    /// @param transferId The ID of the transfer being buffered
    /// @param originDomain The domain where the high-value transfer originated
    /// @param sender The address that initiated the original transfer
    /// @param recipient The intended final recipient of the tokens
    /// @param amount The token amount in source chain units (no decimal scaling)
    /// @param sourceDecimals The number of decimals used by the source token
    /// @param version The MetaERC20 protocol version of the buffered message
    event SecurityRelayBuffered(
        bytes32 indexed transferId,
        uint32 originDomain,
        bytes32 sender,
        address recipient,
    ...
    /// @notice Emitted when a buffered SecurityRelay is approved and re-dispatched as a MintRequest
    /// @param transferId The ID of the original transfer
    /// @param releaseTimestamp The timestamp when the relay was approved by the validator
    /// @param recipientDomain The domain where the tokens will be minted
    /// @param recipient The recipient address for the final minted tokens
    /// @param amount The token amount in source chain units (no decimal scaling)
    event SecurityRelayReleased(
        bytes32 indexed transferId,
    ...
    /// @notice Initializes the MetaERC20Hub with required configuration
    /// @dev Callable only once. Internally calls `_initializeBase` to set MetaERC20 parameters and router config.
    /// Grants `ADMIN_ROLE` to the initializer.
    ...
    /// @param _wrappedToken The address of the canonical ERC20 token this Hub manages
    /// @param _tokenDecimals The number of decimals the wrapped token uses
    function initialize(
        uint32 _localDomain,
        address _metalayerRouter,
        uint256 _ttlWindow,
        ...
        initializer
    {
        ...
        if (_wrappedToken == address(0)) revert InvalidTokenAddress();
        if (_wrappedToken.code.length == 0) {
            revert InvalidWrappedToken(_wrappedToken);
        }
        bytes memory code = _wrappedToken.code;
        if (code.length == 23 && code[0] == 0xef && code[1] == 0x01 && code[2] == 0x00) {
            revert DoaNotContract(_wrappedToken);
        }
        ...
        emit MetaERC20HubInitialized(
            _localDomain, _metalayerRouter, _wrappedToken, _tokenDecimals, _metaERC20Version, _ttlWindow
        );
    }
    ...
    /// @notice Handles an UnlockRequest to release canonical tokens from the Hub
    /// @dev Decodes MetaERC20MessageStruct from raw payload, converts decimal precision,
    ///      and executes a safe transfer of canonical tokens to the recipient.
    ///      Uses CEI pattern: validates recipient, converts decimals, checks balance,
    ///      updates state, then transfers tokens.
    /// @param _originDomain The Metalayer domain ID where the message originated
        ...
        override
    {
        ...
        if (lockedBalance < localAmount) revert LockedBalanceUnderflow();

        lockedBalance -= localAmount;
        wrappedToken.safeTransfer(_message.recipient, localAmount);
        ...
        emit MetaERC20Received(
            ...
            address(this).addressToBytes32() // destinationTokenAddress (Hub contract for now)
        );
    }

    /// @notice Handles a SecurityRelay message for high-value Spoke-to-Spoke transfers
    /// @dev Buffers the relay message for manual approval by VALIDATOR_ROLE before final dispatch.
    ///      Uses dual-mapping pattern for O(1) operations while preserving enumeration.
    ///      Prevents duplicate buffering by checking existing timestamp.
    /// @param _originDomain The Metalayer domain ID where the message originated
    /// @param _senderAddress The address that sent the message on the origin chain (bytes32 format)
    /// @param _metaERC20Version The MetaERC20 protocol version
    /// @param _writeData The raw message payload (already validated and version-matched)
        ...
        override
    {
        ...
        if (_message.recipient == address(0)) revert InvalidRecipient();
        if (relayBuffer[_message.transferId].timestamp != 0) {
            revert DuplicateRelay();
        }

        // Store complete message for later approval
        relayBuffer[_message.transferId] = _message;

        // Update dual-mapping for efficient enumeration and removal
        relayBufferByIndex[relayBufferCount] = _message.transferId;
        relayBufferIndex[_message.transferId] = relayBufferCount;
        relayBufferCount++;

        emit SecurityRelayBuffered(
            _message.transferId,
            _originDomain,
            _senderAddress,
            _message.recipient,
    ...
    /// @notice Returns the balance of the canonical wrapped token held by the specified account
    /// @dev This reflects the balance in the underlying ERC20, not the Hub's locked balance.
    ///      For Hub's locked balance, use `lockedBalance` instead.
    /// @param _account The address to query
    /// @return The balance of `_account` in the wrapped token
    function balanceOf(address _account) external view returns (uint256) {
        return wrappedToken.balanceOf(_account);
    }

    /// @notice Returns the address of the canonical token managed by this Hub
    ...
    /*//////////////////////////////////////////////////////////////
                        RELAY BUFFER FUNCTIONS
    //////////////////////////////////////////////////////////////*/

    /// @notice Returns a paginated slice of relay buffer transfer IDs
    /// @dev Returns empty array if start index exceeds buffer count.
    ///      Automatically clamps end index to prevent out-of-bounds access.
    /// @param start Index to begin reading from (0-based)
    /// @param count Maximum number of IDs to return
    /// @return ids Array of buffered transfer IDs in the requested range
    function listRelayBufferIds(uint256 start, uint256 count) external view returns (bytes32[] memory ids) {
        if (start >= relayBufferCount) {
            return new bytes32[](0);
        }

        uint256 end = start + count;
        if (end > relayBufferCount) end = relayBufferCount;

        ids = new bytes32[](end - start);
        for (uint256 i = start; i < end; i++) {
            ids[i - start] = relayBufferByIndex[i];
        }
    }

    /// @notice Returns the buffered SecurityRelay message for a given transfer ID
    /// @param transferId The ID of the transfer in the relay buffer
    /// @return The buffered MetaERC20MessageStruct, or zeroed if not present
    function getRelayBuffer(bytes32 transferId) external view returns (MetaERC20MessageStruct memory) {
        return relayBuffer[transferId];
    }

    /// @notice Returns buffered relays for a batch of transfer IDs
    /// @param transferIds Array of transfer IDs to query
    /// @return results Array of corresponding buffered relay structs
    function getRelayBufferBatch(bytes32[] calldata transferIds)
        external
        view
        returns (MetaERC20MessageStruct[] memory results)
    {
        if (transferIds.length > MAX_RELAY_QUERY_BATCH) {
            revert ExceedsRelayQueryBatchLimit();
        }

        results = new MetaERC20MessageStruct[](transferIds.length);
        for (uint256 i = 0; i < transferIds.length; i++) {
            results[i] = relayBuffer[transferIds[i]];
        }
    }
    ...
    /// @notice Admin-only emergency function to release funds for a stuck or unacknowledged transfer
    /// @dev Bypasses normal message flow and directly transfers locked tokens to recipient.
    ///      Converts amounts from source decimals to local token decimals.
    ///      Emits MetaERC20Received with originDomain = 0 to signal admin override flow.
    /// @param _transferId The ID of the original transfer to recover from storage
    /// @param _recipientAddress The address that will receive the unlocked tokens
    function EMERGENCY_UNLOCK(bytes32 _transferId, address _recipientAddress) external onlyRole(ADMIN_ROLE) {
        ...
        MetaERC20MessageStruct memory message = _getTransferRecord(_transferId);
        if (message.amount == 0) revert TransferNotFound();

        uint256 localAmount = _convertDecimals(message.amount, message.sourceDecimals, tokenDecimals);
        if (lockedBalance < localAmount) revert LockedBalanceUnderflow();

        _deleteTransfer(_transferId);
        lockedBalance -= localAmount;

        wrappedToken.safeTransfer(_recipientAddress, localAmount);

        emit MetaERC20Received(
            _transferId,
            0, // originDomain unknown in admin flow
            msg.sender.addressToBytes32(),
            localDomain,
            ...
            address(wrappedToken).addressToBytes32() // destinationTokenAddress
        );

        emit MetaERC20AdminUnlock(_transferId, msg.sender, _recipientAddress);
    }

    /// @notice Validates and approves a buffered SecurityRelay, dispatching it as a MintRequest
    /// @dev Removes relay from buffer using O(1) swap-and-pop algorithm, then dispatches
    ///      the message to the original recipient domain as a MintRequest for final minting.
    /// @param transferId The ID of the buffered SecurityRelay to approve
    function APPROVE_SECURITY_RELAY(bytes32 transferId) external payable onlyRole(VALIDATOR_ROLE) {
        MetaERC20MessageStruct memory relay = relayBuffer[transferId];
        if (relay.timestamp == 0) revert TransferNotFound();

        // Get the index of the item to remove
        uint256 indexToRemove = relayBufferIndex[transferId];
        uint256 lastIndex = relayBufferCount - 1;

        if (indexToRemove != lastIndex) {
            // Move last item to the position being removed
            bytes32 lastTransferId = relayBufferByIndex[lastIndex];
            relayBufferByIndex[indexToRemove] = lastTransferId;
            relayBufferIndex[lastTransferId] = indexToRemove;
        }

        // Clean up
        delete relayBufferByIndex[lastIndex];
        delete relayBufferIndex[transferId];
        delete relayBuffer[transferId];
        relayBufferCount--;

        bytes32 destinationRouterAddress = metaERC20Addresses[relay.recipientDomain];
        if (destinationRouterAddress == 0) {
            revert MetaERC20NotRegistered(relay.recipientDomain);
        }

        bytes memory message = MetaERC20Message.encodeMessage(
            MetaERC20MessageStruct({
                transferId: relay.transferId,
                timestamp: relay.timestamp,
                metaERC20Version: relay.metaERC20Version,
                messageType: MetaERC20MessageType.MintRequest,
                recipientDomain: relay.recipientDomain,
                recipient: relay.recipient,
                amount: relay.amount,
                sourceDecimals: relay.sourceDecimals
            })
        );

        metalayerRouter.dispatch{ value: msg.value }(
            relay.recipientDomain, destinationRouterAddress, new ReadOperation[](0), message, finalityState, gasLimit
        );
        ...
        emit MetaERC20Dispatch(
            ...
            relay.recipient.addressToBytes32(),
            relay.amount,
            relay.sourceDecimals,
            MetaERC20MessageType.MintRequest,
            relay.metaERC20Version,
            finalityState,
            gasLimit,
            bytes32(0), // sourceTokenAddress unknown in admin flow
            metaERC20Addresses[relay.recipientDomain] // destinationTokenAddress
        );

        emit SecurityRelayReleased(transferId, block.timestamp, relay.recipientDomain, relay.recipient, relay.amount);
    }
    ...
    /// @notice Determines the dispatch parameters for a Hub-originating transfer
    /// @dev Hub always emits MintRequest messages directly to the destination Spoke's domain.
    ///      This is the simplest routing logic: Hub locks tokens and tells Spoke to mint.
    /// @param _recipientDomain The originally requested destination domain
    /// @param _message The partially constructed MetaERC20MessageStruct for this transfer (unused in Hub)
    /// @return messageType Always returns MetaERC20MessageType.MintRequest for Hub behavior
    /// @return destinationRouterAddress The router address of the Spoke contract on the destination domain
    /// @return destinationDomain Always equals _recipientDomain for Hub (no rerouting needed)
    function _resolveDispatchArguments(
        uint32 _recipientDomain,
        MetaERC20MessageStruct memory _message
    )
        ...
        returns (MetaERC20MessageType messageType, bytes32 destinationRouterAddress, uint32 destinationDomain)
    {
        ...
        return (MetaERC20MessageType.MintRequest, destinationRouterAddress, _recipientDomain);
    }

    /**
     * @notice Returns the source token address for events (wrappedToken for Hub)
     * @return The wrapped token address as bytes32
     */
    function _getSourceTokenAddress() internal view override returns (bytes32) {
        return address(wrappedToken).addressToBytes32();
    }

    /// @notice Locks tokens by transferring them from the sender to this contract
    /// @dev Called automatically before dispatching a cross-chain message from the Hub.
    ///      Increments locked balance tracking and performs safeTransferFrom of canonical tokens.
    ///      Uses CEI pattern: updates state before external call for reentrancy protection.
    /// @param _sender The address initiating the transfer
    /// @param _amount The token amount to lock in local token units (no decimal scaling)
    ...
    /// @notice Validates that the Hub's locked balance accounting matches the actual token holdings
    /// @dev Used for operational monitoring and emergency detection. The locked balance should never
    ///      exceed the actual token balance held by the contract. A discrepancy could indicate:
    ///      - Direct token transfers to the contract (positive discrepancy)
    ///      - Accounting bugs or external token removal (negative discrepancy)
    /// @return isValid True if lockedBalance <= actual token balance, false if accounting error detected
    function validateLockedBalance() external view returns (bool isValid) {
        return lockedBalance <= wrappedToken.balanceOf(address(this));
    }

    /// @notice Returns the difference between actual token balance and tracked locked balance
    /// @dev Positive values indicate excess tokens (e.g., direct transfers to contract).
    ///      Negative values indicate a critical accounting error where we think we have more
    ///      locked than we actually hold. Zero indicates perfect accounting alignment.
    /// @return discrepancy Signed difference (actual_balance - locked_balance)
    ///         - Positive: Contract holds more tokens than tracked
    ///         - Negative: Critical error - locked balance exceeds actual holdings
    ///         - Zero: Perfect accounting alignment
    function getBalanceDiscrepancy() external view returns (int256 discrepancy) {
        return int256(wrappedToken.balanceOf(address(this))) - int256(lockedBalance);
    }
