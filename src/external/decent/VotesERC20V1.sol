...
// contracts/interfaces/decent/IDeploymentBlock.sol
...
interface IERC6372 {
    /**
     * @dev Clock used for flagging checkpoints. Can be overridden to implement timestamp based checkpoints (and
     * voting).
     */
...
// contracts/interfaces/decent/deployables/IVersion.sol
 ...
 * number. Used throughout the Decent Protocol to track contract versions for
...
// contracts/interfaces/decent/deployables/IVotesERC20V1.sol
...
// contracts/InitializerEventEmitter.sol
...
library SignedMath {
    ...
    function abs(int256 n) internal pure returns (uint256) {
        unchecked {
            ...
            // result, the mask will either be `bytes32(0)` (if n is positive) or `~bytes32(0)` (if n is negative).
            int256 mask = n >> 255;
...
// contracts/DeploymentBlockInitializable.sol
...
abstract contract DeploymentBlockInitializable is Initializable, IDeploymentBlock {
     ...
     * keccak256(abi.encode(uint256(keccak256("Decent.DeploymentBlockInitializable.main")) - 1)) &
...
library Strings {
    ...
    /**
     * @dev Variant of {parseUint-string} that parses a substring of `input` located between position `begin` (included)
     * and
     * `end` (excluded).
     *
     * Requirements:
    ...
    /**
     * @dev Implementation of {tryParseUint-string-uint256-uint256} that does not check bounds. Caller should make sure
     * that
    ...
    /**
     * @dev Variant of {parseInt-string} that parses a substring of `input` located between position `begin` (included)
     * and
     * `end` (excluded).
     *
     * Requirements:
    ...
    /**
     * @dev Implementation of {tryParseInt-string-uint256-uint256} that does not check bounds. Caller should make sure
     * that
     * `begin <= end <= input.length`. Other inputs would result in undefined behavior.
     */
    ...
    /**
     * @dev Variant of {parseHexUint-string} that parses a substring of `input` located between position `begin`
     * (included) and
     * `end` (excluded).
     *
     * Requirements:
    ...
    /**
     * @dev Implementation of {tryParseHexUint-string-uint256-uint256} that does not check bounds. Caller should make
     * sure that
     * `begin <= end <= input.length`. Other inputs would result in undefined behavior.
     */
    ...
    /**
     * @dev Variant of {parseAddress-string} that parses a substring of `input` located between position `begin`
     * (included) and
     * `end` (excluded).
     *
     * Requirements:
...
library MessageHashUtils {
     ...
     * hash signed when using the https://ethereum.org/en/developers/docs/apis/json-rpc/#eth_sign[`eth_sign`] JSON-RPC
     * method.
     *
...
/**
 * @dev Extension of ERC-20 to support Compound-like voting and delegation. This version is more generic than
 * Compound's,
 ...
 * by calling the {Votes-delegate} function directly, or by providing a signature to be used with {Votes-delegateBySig}.
 * Voting
...
abstract contract ERC20VotesUpgradeable is Initializable, ERC20Upgradeable, VotesUpgradeable {
     ...
     * {Votes-_transferVotingUnits}. An override could be used to further restrict the total supply (to a lower value)
     * if
...
// contracts/deployables/erc20/VotesERC20V1.sol
    ...
    ERC165
{
     ...
     * keccak256(abi.encode(uint256(keccak256("Decent.VotesERC20.main")) - 1)) & ~bytes32(uint256(0xff))
     */
