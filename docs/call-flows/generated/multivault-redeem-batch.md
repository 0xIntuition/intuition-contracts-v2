...
# MultiVault.redeemBatch

Batched redeem; each leg runs the same `_processRedeem` body as the single-term path.

Reached functions: 32. Solid arrows are calls inside one contract; dashed arrows leave it (either a `delegatecall` into
...
flowchart LR
  ...
  subgraph MultiVault["MultiVault"]
    MultiVault_redeemBatch["redeemBatch"]
  end
  subgraph MultiVaultLib["MultiVaultLib"]
    ...
    MultiVaultLib_redeemBatch["redeemBatch"]
  end
  MultiVault_redeemBatch -.-> MultiVaultLib_redeemBatch
  MultiVaultLib_redeemBatch --> MultiVaultLib__isApprovedToRedeem
  MultiVaultLib_redeemBatch --> MultiVaultLib__processRedeem
  MultiVaultLib_redeemBatch --> MultiVaultLib__removeUtilization
  ...
  style MultiVault_redeemBatch stroke-width:3px
```
