...
| **MA-02**   | **Open**                                                              | The redistribution shape is confirmed intentional. On deposit, a configurable portion of the fee is paid to the nearest occupied prior tier and the remainder is spread across prior tiers by the sliding fulcrum kernel according to the configured split. On withdrawal, the fee is paid to the exiting holder's own tier; where that tier has no remaining occupant it is routed to the nearest occupied tier, searched upward first and then downward; a configurable portion may additionally be routed to the fulcrum, mirroring the deposit side. Allocation between tiers by kernel weight is therefore the designed mechanic, and the recommendation to make it stake-proportional is **not** carried forward on that basis. The recommendation to re-derive `userTier` / `userAvgTier` on redeem is separately **superseded**: it would allow a holder to reduce their recorded tier by splitting withdrawals and thereby pay a lower withdrawal rate, which is a worse outcome than the behavior it addresses. **What remains open** is narrow and economic rather than structural: whether stake below a minimum threshold should be excluded from earning and from recipient denominators, so that a negligible position cannot receive a slice sized for a full cohort. This is under evaluation; it is not a correctness defect, and conservation and solvency hold in either configuration.                                                                                                                                                                                                                                                                                                                   |
...
**Summary — 35 findings, all dispositioned. 34 closed, 1 open.**
...
| **Closed — By design**      | 11    | MA-01, MA-03, MA-04, MED-05, MED-07, MIN-07, MIN-10, MIN-11, INFO-04, INFO-10, and the MA-02 kernel shape |
| **Closed — Acknowledged**   | 10    | MED-06, MED-08, MIN-05, MIN-08, MIN-09, INFO-01, INFO-02, INFO-05, INFO-06, INFO-07, INFO-09              |
| **Open**                    | 1     | MA-02 — the minimum-eligible-stake question only                                                          |

The single Open item is a scoped economic question — whether stake below a minimum threshold should be excluded from
earning and from recipient denominators — not a correctness defect. Conservation and solvency hold in either
configuration.

**Every fix carries a gating regression test that was mutation-checked**: the guard was removed, the test confirmed red,
the guard restored, the test confirmed green. A test green in both states was not accepted as coverage. The gating test
is named in each register row above.
...
**Twelve findings closed as `Fixed`.** Ten landed as behavioural or test changes in this remediation pass; two
(`MIN-12`, `INFO-03`) landed earlier as documentation. Every behavioural fix carries a gating regression test that was
**mutation-checked** — guard removed, test confirmed red, guard restored, test confirmed green — and every gating test
is named in its register row. Test names are provenance-free.
