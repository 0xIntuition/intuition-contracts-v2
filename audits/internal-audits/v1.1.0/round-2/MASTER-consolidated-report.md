...
| ----------- | ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MA-01**   | **Open**                                                   | Not live — verified on chain `1155`: `defaultCurveId == 1` and the registry holds 2 curves (the dynamic-fee curve is absent). `defaultCurveId` is **not intended to change**; the design is that hook curves are reachable only through `deposit` / `redeem` on an explicit non-default `curveId`. **Docs landed** (`319303f`): `IBaseCurve.recordDeposit` NatSpec previously asserted the hook "is ALWAYS called" — it now states the creation-path exception explicitly and that a hook-bearing curve must not be `defaultCurveId`. **Residual, and what closes this:** harden `setBondingCurveConfig` to pin `defaultCurveId` (or reject a target advertising either hook), with a gating test. Owner: protocol team. Reviewer option (a) — dispatch the hook from the create paths — is **not** being taken: creation is deliberately hook-free.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **MA-02**   | **Open**                                                   | Two separable parts. **(a) Dust-tier participation** — a tier whose entire stake is dust is a full-weight recipient. Agreed fix: gate tier participation and recipient denominators on a **minimum eligible stake**, so sub-threshold stake neither earns nor dilutes. `generalConfig.feeThreshold` is `1e18` (1 TRUST) on chain, which is the right order of magnitude and kills both PoC positions (1 wei and 0.1 TRUST); prefer a **dedicated curve-config field** over reading MultiVault's, since `feeThreshold` is defined as a _default-curve vault total_, not a per-user floor. **(b) Whether per-bucket lump allocation is itself intended** — unresolved; needs the economic modeling answer before any change to `_weighPriorTiers`. **Reviewer recommendation to re-derive `userTier` / `userAvgTier` on redeem is SUPERSEDED on economic grounds:** it would let a holder salami-slice withdrawals to walk their average tier down and pay a lower withdrawal rate, which is strictly worse than the behavior it fixes.                                                                                                                                                                                                                                                                                                                                                                                 |
| **MA-03**   | **Closed — By design**                                     | The core mechanic — the exiting tier's _remaining_ occupants receive the withdrawal fee, split pro-rata among them, with the exiter excluded from the fee they themselves pay — **is the intended design**. Self-exclusion is implemented and was verified directly at `DynamicFeeFlatPriceCurve.sol:401-452`: `_settle` banks pending, the exiting stake is removed from `tierStake` _before_ `denom = tierStake[exitTier] - residual` is computed, and `rewardDebt` is re-based against the post-distribution accumulator. A sole remaining occupant receiving the whole slice is the mechanic working, not failing. **Asserted by** the corrected `recordRedeem` / contract-header NatSpec landed in `319303f`. The residual concern is only the _magnitude_ when the remaining occupant holds dust — that is bounded by the minimum-eligible-stake floor tracked under `MA-02(a)`, and is not double-counted here. Reviewer dwell/re-derive recommendations superseded — see `MA-02` and `MA-04`.                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **MA-04**   | **Closed — By design**                                     | **Superseded by a committed test.** `tests/unit/MultiVault/DynamicFeeAdversarialEconomics.t.sol::test_frontRun_sandwichEarningsBoundedByVictimFee` asserts the bound (`attackerEarned <= victimFee`) and its NatSpec states the intent in terms: _"Earning from later depositors is the intended mechanic; the invariant is that it stays within collections."_ This is an **activity-driven redistribution, not a yield-accrual product** — a dwell requirement, time-weighting, or minimum holding period is deliberately **not** wanted, so the reviewer's `youngStake` proposal is rejected, not deferred. Conservation binds: no party earns more than the fees actually collected, and the +221% figure is a return on the attacker's own capital out of a fee that was going to be redistributed regardless — no principal is taken. **Docs landed** (`319303f`): the "diamond-hands" framing in the contract header and `IDynamicFeeFlatPriceCurve` overstated a commitment guarantee the code does not implement and has been replaced with an explicit no-dwell statement. Concentration is further bounded by `MA-02(a)`'s floor. **Note the honest limit:** the existing test covers the _deposit_ side; the withdrawal side, where the shipped `withdrawalToRecentShareBps = 0` routes 100% of the fee to one tier, has no equivalent test. Adding the withdrawal-side mirror is tracked under `MIN-02`. |
| **MA-05**   | **Open**                                                   | **Quantified.** The shipped schedule sets `withdrawalCapBps = 1000` (10%), not `BPS`. Reaching a zero payout therefore requires **two deliberate governance actions** — raise the cap roughly tenfold, then set a rate inside it — not one. **What closes this:** (1) move curve ownership to the parameters `TimelockController` (`MED-01`), which makes both actions timelocked; (2) add an **immutable ceiling** on `withdrawalCapBps` so no configuration, timelocked or not, can store a schedule capable of consuming a redemption. Note the reviewer's framing that a settable cap is missing is inaccurate — `config.withdrawalCapBps` already exists and is settable; what is missing is a bound _on the setter_. (3) The `MultiVault` payout floor is accepted as **defense in depth**, not as the required fix, since any sane schedule respects it. **Docs landed** (`319303f`): the `setTierFeeOverride` NatSpec previously described the failure mode as a revert; it now states the damaging mode is a silent zero payout at rates strictly inside the cap.                                                                                                                                                                                                                                                                                                                                            |
| **MED-01**  | **Open**                                                   | Agreed and scoped to the deploy script: `script/intuition/DeployDynamicFeeFlatPriceCurve.s.sol:80` passes `msg.sender` as owner with an inline PoC-placeholder comment. It must pass the **parameters `TimelockController`** on mainnet chain ids. **Reviewer recommendation to add an `onlyTimelock` modifier is SUPERSEDED:** the curve is `Ownable` with no role system, so `owner == parametersTimelock` delivers identical gating without introducing a second access mechanism. Closing this also closes `MIN-09` (`renounceOwnership` becomes timelocked). Owner: protocol team.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **MED-02**  | **Closed — Acknowledged**                                  | **Quantified.** Shipped curve caps are `depositCapBps = 1000` and `withdrawalCapBps = 1000` (10% each); on-chain vault fees are entry `50`, exit `75`, protocol `125` over denominator `10000` (0.5% / 0.75% / 1.25%). Combined worst case under the shipped configuration is ≈**11.75% against a 100% ceiling** — roughly 88 points of headroom. The measured brick thresholds (9901 bps redeem-side, 9990 bps deposit-side) require raising a cap by nearly an order of magnitude. Accepted as an operator footgun, not a defect: the envelope check is genuine defense in depth and is not required for any sane schedule. **Ops runbook consequence:** any `setConfig` raising either cap above ~8000 bps must be checked against the live vault fee schedule before submission.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **MED-03**  | **Open**                                                   | Severity **downgraded to Minor** — no on-chain value is lost and execution paths are internally consistent. **The reviewer's recommendation to add an `account` parameter to `MultiVault.previewRedeem` is SUPERSEDED:** it would push one curve's tier semantics into the shared vault ABI, and the same problem recurs for every future curve with its own per-user state. The correct home is the curve. Note `IBaseCurve.quoteRedeemFee(termId, account, assets)` **already takes an account** and already returns the holder-accurate figure — the gap is that this is undocumented and that `MultiVault.previewRedeem` gives no warning. **What closes this:** (1) NatSpec on `IMultiVault.previewRedeem` stating it is account-agnostic and must not be used to derive `minAssets` on a hook curve — **docs, pending**; (2) optionally a curve-side `previewRedeemFor(termId, account, shares)` convenience view.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **MED-04**  | **Open** — _disagreement recorded_                         | The mechanism is confirmed by direct source inspection: `_depositFeeBps` / `_withdrawalFeeBps` return a set override **unclamped**, skipping the `fee < cap ? fee : cap` clamp the computed branch applies, so lowering a cap does not tighten a tier that already carries an override. **Documented as an interim** in `319303f`. The protocol team's position is that this is Informational, being timelock-only and reconfiguration-driven. **This report's position is that it is worth the three-line read-time clamp**, because it is the one finding where a guard _fails open on an action whose intent is to tighten_ — precisely the operator error that `MA-05` depends on. Recorded as unresolved rather than silently applied.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **MED-05**  | **Closed — By design** (+ docs)                            | The 1:1 default-curve assumption is deliberate and load-bearing. **Correction to the assumption that it was already documented — it was not:** `_getAtomCost` / `_getTripleCost` carried only "the static costs of creating an atom", with no mention that `minShare` is a _share_ quantity added to an _asset_ quantity while the create path funds with `_minAssetsForCurve(...)` _assets_. **Docs landed** (`319303f`) recording the assumption, that it holds by construction for `defaultCurveId == 1`, and that it must be re-derived if that ever changes. The setter-hardening half (pin `defaultCurveId`, reject `address(0)`, reject a hook-bearing target) is **not duplicated here** — it is tracked once, under `MA-01`. Adding broad per-field validation to `setBondingCurveConfig` is explicitly rejected as scope creep.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **MED-06**  | **Closed — Acknowledged**                                  | **Verified on chain `1155`:** `currentEpoch() == 19` and `epochLength() == 1209600` (14 days), so the genesis-epoch branch is long past; and `_initialized == 1` on both the MultiVault and TrustBonding proxies, confirming `reinitializer(2)` has not executed. Branch (a) requires an upgrade epoch with **zero** MultiVault activity, which is trivially avoidable and, if it ever occurred, is self-correctable by the protocol generating activity itself (including an atomic unpause → activity → pause batch). Branch (b) — the permissionless pre-`reinitialize` window — is accepted: leaving that window unpopulated is explicitly not a requirement. **Ops runbook consequence:** warm the upgrade epoch with any MultiVault activity before `upgradeAndCall`, and run the reinitializers in the same operational window. Carried into §8. **One sub-question remains genuinely open** (`R2-CYF-44`): whether `TrustBonding` consumes a negative `getTotalUtilizationForEpoch` safely. Cheap to answer and should be answered during remediation.                                                                                                                                                                                                                                                                                                                                                        |
| **MED-07**  | **Closed — By design**                                     | **Superseded by a committed test.** `DynamicFeeAdversarialEconomics.t.sol::test_liveFeeChange_doesNotRepriceAccruedBalances` asserts that retuning must not re-price already-accrued balances — which is the guarantee that matters. A retune changing forward rates (e.g. a tier's deposit fee moving 10% → 12%) applies only to subsequent activity; nothing is done retroactively and nothing is intended to be. Index-keyed accumulators surviving a retune is a consequence of that same choice. **Docs pending:** a short note on `setConfig` recording that a retune re-targets the forward fee stream and does not migrate positions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **MED-08**  | **Closed — Acknowledged**                                  | **With a correction to the reported mechanism.** The CREATE2 salt is `atomId` alone, but the address also depends on `keccak256` of the initcode, which embeds **all three** of `entryPoint`, `multiVault` and `atomWalletBeacon` (`AtomWalletFactory._getDeploymentData`) — not `entryPoint` alone. The important distinction for operations: **upgrading the beacon's implementation is safe** (the beacon _address_ is unchanged); rotating the `entryPoint` or pointing `walletConfig` at a _different beacon contract_ re-derives every not-yet-deployed wallet address. Already-deployed wallets keep their addresses and keep working. The protocol does not intend to rotate the EntryPoint. Accepted under the trust model. **Docs pending:** record the coupling and the ops rule on `walletConfig` / the factory.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **MIN-01**  | **Open**                                                   | Agreed as a small, contained fix: split the view so the per-term and cross-vault components are separately addressable (`pendingFor`, `bankedEarnings`, `claimableAcross`), keeping `claimable` for ABI compatibility with a NatSpec warning. No on-chain value is affected — `claim` zeroes `earned` exactly once. Owner: protocol team.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **MIN-02**  | **Open**                                                   | Test-only. The quote-then-record fee-parity property is load-bearing for the whole C2 hook surface and was mutation-proved uncovered (guarantee removed → 28 of 28 dynamic-fee tests still pass). Add a gating test that goes red on that mutation. Should also carry the **withdrawal-side** front-run mirror noted under `MA-04`, which the existing deposit-side sandwich test does not cover.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **MIN-03**  | **Open**                                                   | Test-only, and **not** a deploy-script item — this is the ERC-1271 negative regression for round-1 `MED-01`. The fix itself is present and correct; what is missing is a test asserting a _bare_ digest is **rejected**, so that a permissive "keep old signatures working" refactor goes red. Mutation-proved: under that permissive mutation all 122 pre-existing wallet tests stay green. Adopt the two delivered negative tests as gating.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **MIN-04**  | **Open** — _partial reopen of round-1 `INFO-04`_           | There **is** something actionable, and it is not a comment: the layout suite must anchor each config-region slot against a **raw literal**. A getter-relative assertion cannot detect an intra-struct reorder in principle, because the slot and the auto-getter's return position move together; the library mirror cannot detect it either, since both sides re-use the same struct type and move in lockstep. Mutation-proved green/green (swap `minDeposit` / `minShare` → all 11 tests still pass). Round-1 `INFO-04` was closed by rewording NatSpec to claim the CI suite pins the layout; that claim is true at slot level and false for intra-struct order.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **MIN-05**  | **Closed — Acknowledged**                                  | Agreed with the protocol team's position: it is up to each curve to declare which hooks it supports. The asymmetric case fails **closed** and is not permissionlessly reachable — `BondingCurveRegistry.addBondingCurve` is `onlyOwner`, and `BaseCurve` defaults both getters to `false` with `quoteDepositFee` / `quoteRedeemFee` reverting, so a curve must be deliberately written _and_ deliberately registered to be malformed. That is "a trusted admin can do X", which is out of scope.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **MIN-06**  | **Open**                                                   | Confirmed real by direct comparison. `AtomWallet.executeBatch(address[],uint256[],bytes[])` carries `onlyMultiOwnableOwnerOrEntryPoint` **and** `nonReentrant`; the inherited `ERC4337.executeBatch(Call[])` (solady) carries only `onlyEntryPointOrOwner` — a **different owner notion** from the protocol's MultiOwnable set — and **no reentrancy guard**. Agreed fix: override the inherited selector. Touches an auth path, so it is a code change with a gating test, not a docs item. Owner: protocol team.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **MIN-07**  | **Closed — Not applicable** — _becomes a pre-upgrade gate_ | **Verified on chain `1155`:** `AtomWarden.claimWindow()` **reverts** on the deployed implementation — the creator-expiry fallback path does not exist on the live system, so no wallet can be pre-empted today. The behavior is intended: a deliberately long window (order of a year) exists so that an unclaimed wallet's accrued fees are not stranded forever, and `claimWindow == 0` disables the path entirely. **Because the path ships with this upgrade, the point-in-time fact is load-bearing:** the deployed `claimWindow` must be set deliberately at upgrade time. Carried into §8 as a gate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **MIN-08**  | **Closed — Acknowledged**                                  | Liveness only, never authorization — the failure modes are a quorum that cannot be met, not a quorum that can be bypassed. Managed operationally. **Ops runbook consequence:** never grant `SIGNER_ROLE` to `address(0)`, and be aware that recovering from a fully-renounced signer set requires a `grantRole` before `setSignatureThreshold` can be lowered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **MIN-09**  | **Closed — Acknowledged**                                  | Closes as a consequence of `MED-01`: once the curve and registry owner is the parameters `TimelockController`, `renounceOwnership` is itself timelocked and subject to the 4-of-8 Safe. No separate change.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **MIN-10**  | **Closed — By design**                                     | A holder whose own address reverts on receiving native value is not a case the protocol undertakes to serve. `claim` pays the caller; there is no recipient indirection and none is wanted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **MIN-11**  | **Closed — By design**                                     | **Superseded by a committed test, and the finding's magnitude claim does not survive measurement.** `DynamicFeeAdversarialEconomics.t.sol::test_sybil_splitReceiversAreEconomicallyNeutral` splits `24e18` into two `12e18` deposits and asserts `assertApproxEqRel(sharesA + sharesB, sharesSingle, 0.001e18)` — i.e. **any splitting advantage is under 0.1%**, and its NatSpec states the reasoning: fees are piecewise on the _vault trajectory_, not on the actor's wallet count. `_piecewiseDepositFee` walks tier bands from the vault's current position, so the only residual asymmetry is that a chunked second leg starts from a marginally lower cursor — and that is partly offset by an extra `mulDivUp` round-up per chunk. Well below the gas cost of operating extra wallets.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **MIN-12**  | **Closed — Fixed** (docs)                                  | **Landed this session, commit `b9510ab`** — 15 lines across 13 tracked test files under `tests/unit/security/v1.1.0/` and `tests/unit/upgrades/v1.1.0/`. Removed: an internal issue-tracker identifier (6 files), sequential `Track N —` workstream labels (6 files), `Hypothesis N (deep pre-audit)` round labels, and `pre-audit` qualifiers in the upgrade-regression titles. Every title was rewritten to name the mechanism under test; no test logic changed and all 108 tests in those two directories pass. Re-grep confirms zero residual markers. **Gating regression pending:** a CI grep for provenance markers under `tests/`, which is what makes this durable rather than a one-time cleanup.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **INFO-01** | **Closed — Acknowledged**                                  | The share/asset unit coupling is real and load-bearing, and holds by construction today: entry and exit fees route to the _default_ curve's vault, so a non-default-curve vault never drifts from 1:1. Accepted. **Docs pending:** a note on `vaultAssets` recording that the tier ladder is asset-denominated while the mirror accumulates shares, and that the two agree only at par. An invariant test pinning `currentSharePrice(term, dynamicCurveId) == 1e18` is the cheap durable guard and is recommended alongside `MIN-02`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **INFO-02** | **Closed — Acknowledged**                                  | Gas and design hygiene only. Correctness was actively confirmed safe by the reporting round: both quotes are `STATICCALL`s with only another `STATICCALL` between them, and an adversarial curve returning different answers across _transactions_ is rejected by the slippage guard rather than slipping through. Not worth a write-path change at this stage.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **INFO-03** | **Closed — Fixed** (docs)                                  | **Landed this session, commit `319303f`.** The `setTierFeeOverride` NatSpec claimed an over-large withdrawal rate "underflows `assets - fees` in MultiVault, bricking redeems … until retuned" — i.e. a revert with funds safe. It now states that the revert occurs only at exactly `BPS`, and that at any rate strictly inside the cap the redemption **succeeds and pays zero** while burning shares, with `previewRedeem` reporting the same zero without erroring. This was the specific line that would have misled an operator into believing `MA-05` was a safe failure.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **INFO-04** | **Closed — By design**                                     | Correct behavior, not a gap. When a vault sits in tier 0 there is no prior tier to redistribute to, and for the last redeemer in a vault there is no remaining cohort at all — the protocol absorbing the fee is the intended terminal case, and the redeem path already handles the structurally similar condition via `_nearestOccupiedTier`. The NatSpec word "dust" understates the tier-0 case; folded into the `INFO-01` docs pass.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **INFO-05** | **Closed — Acknowledged**                                  | **Quantified and directionally safe.** Measured drift over a 16-operation sequence was `0 → −13 → −30 → −92 → −143` wei — monotone, dust-class, and in the **conservative** direction (custody exceeds obligations), which strengthens solvency rather than threatening it. The related `+1` wei transient can revert a single `claim()` until the next fee credit. Accepted as the standard MasterChef dust residue. A residual sweep is optional cleanup, not a fix.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **INFO-06** | **Closed — Acknowledged**                                  | The re-entrancy path is closed today by the outer `MultiVault` `nonReentrant` guard, confirmed by three rounds including an adversarial curve that attempts the re-entry and is rejected; the hooks are additionally `onlyMultiVault`, so there is no unprivileged caller. Adding `nonReentrant` to the record hooks is cheap belt-and-braces and is **recommended but not required** — its real value is protecting the ordering assumption against a future edit, which a comment cannot do.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **INFO-07** | **Closed — Acknowledged**                                  | Declining to change `CoreEmissionsController`. Declaring the mixin `abstract` is correct in the abstract, but the contract sits on the emissions path, the artifact is never deployed standalone, and the change buys nothing operationally against a non-zero risk of disturbing a controller that is already live and initialized. Revisit only if the file is being modified for another reason.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **INFO-08** | **Closed — Not applicable** — _pre-upgrade gate_           | Round-1 `MA-01`'s disposition **stands**; the override filed against it failed its own round's verification (the deployed v1.0.2 claim path is self-only, cannot be triggered against a third party, and creating the precondition means self-bricking for no gain). The census remains: one AtomWallet ever deployed, unclaimed, and the legacy claim surface is effectively protocol-controlled. **Because no code-level migration was built, that fact is load-bearing** — carried into §8 as a re-check immediately before the beacon upgrade, per the runbook rule. The recommended on-chain pre-flight assertion is the cheap way to convert it from an ops assumption into an enforced gate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **INFO-09** | **Closed — Acknowledged**                                  | Matches round-1 `INFO-01`'s disposition and rationale, broadened to three encodings. **Not exploitable here:** ERC-1271 validation is stateless — there is no nonce, no `usedSignature` store, and nothing keyed on signature bytes anywhere in the system — so a malleated variant grants exactly what the original grants and nothing more; UserOp replay is prevented by EntryPoint nonces. High-`s` and EIP-2098 acceptance are inherited from a production-proven upstream library and are deliberately **not** changed. The one axis with a genuinely trivial tightening is the trailing-padding variant — `CoinbaseSmartWalletLib.isValidSignature` checks `signature.length < 96 + paddedDataLen`, a **minimum** rather than an exact match. Changing that comparison to `!=` is a one-operator strict tightening that every in-repo producer already satisfies. Optional; treated as a code change on an auth path, so it needs an SDK compatibility sweep first.                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **INFO-10** | **Closed — By design**                                     | Not an issue. The round-1 `MED-02` guard rejects `emissionsLength == 0` because zero permanently breaks epoch math; there is no comparable failure mode at the upper end, and an arbitrary ceiling would be a guess. Both live controllers are initialized to a 14-day epoch (verified on chain `1155`: `1209600`) with no setter.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |

**Summary — 35 findings, all dispositioned.**

| Status                      | Count | IDs                                                                                                  |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------- |
| **Closed — Fixed** (docs)   | 2     | MIN-12, INFO-03                                                                                      |
| **Closed — Not applicable** | 2     | MIN-07, INFO-08 — _both become pre-upgrade gates_                                                    |
| **Closed — By design**      | 8     | MA-03, MA-04, MED-05, MED-07, MIN-10, MIN-11, INFO-04, INFO-10                                       |
| **Closed — Acknowledged**   | 12    | MED-02, MED-06, MED-08, MIN-05, MIN-08, MIN-09, INFO-01, INFO-02, INFO-05, INFO-06, INFO-07, INFO-09 |
| **Open**                    | 11    | MA-01, MA-02, MA-05, MED-01, MED-03, MED-04, MIN-01, MIN-02, MIN-03, MIN-04, MIN-06                  |

**Code changes vs documentation changes, separated per the runbook.** Nothing landed in this pass altered behavior.

- **Landed — documentation and test-title only** (2 commits, no behavior change, full suite green): `b9510ab`
  (provenance markers stripped from 13 tracked test files) and `319303f` (NatSpec on `IBaseCurve.recordDeposit`,
  `setTierFeeOverride`, the contract-header tier-entitlement framing,
  `IDynamicFeeFlatPriceCurve.withdrawalToRecentShareBps`, and `MultiVaultCore._getAtomCost` / `_getTripleCost`).
- **Still pending as documentation only:** `MED-03` (preview warning), `MED-07` (retune note), `MED-08` (wallet-address
  coupling), `INFO-01` (unit coupling + `INFO-04` tier-0 wording).
- **Pending as code changes** (behavior, storage, or auth — each needs its own gating, mutation-checked regression
  test): `MA-01` setter hardening, `MA-02(a)` minimum-eligible-stake floor, `MA-05` immutable cap ceiling (+ optional
  MultiVault payout floor), `MED-01` deploy-script owner, `MED-04` read-time override clamp (disputed), `MIN-01` view
  split, `MIN-06` `executeBatch` override, `INFO-09` exact-length check (optional).
- **Pending as test-only work:** `MIN-02`, `MIN-03`, `MIN-04`, plus the `MIN-12` CI grep.
- **Blocked on a non-engineering input:** `MA-02(b)` — whether stake-independent between-tier allocation was a
  deliberate choice in the economic model the curve was ported from.
...
**All 35 findings are dispositioned — see the register at §5b. 24 are closed; 11 remain Open.** The register is the
source of truth from here.

**Nothing in this pass changed behavior.** Two commits landed, both documentation or test-title only, and the honesty
rule is applied strictly: no finding is marked `Closed — Fixed` on the strength of a code change, because no code change
was made.

| Commit    | Kind                   | Contents                                                                                                                                                                                                                                                                                                             | Closes                                                                          |
| --------- | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `b9510ab` | Test titles (no logic) | Internal provenance markers stripped from 13 tracked test files, 15 lines. All 108 tests in the two affected directories pass; re-grep confirms zero residual markers.                                                                                                                                               | `MIN-12`                                                                        |
| `319303f` | NatSpec (no behavior)  | `IBaseCurve.recordDeposit` creation-path exception; `setTierFeeOverride` corrected failure mode and unclamped-override note; contract-header tier-entitlement framing; `IDynamicFeeFlatPriceCurve.withdrawalToRecentShareBps`; `MultiVaultCore._getAtomCost` / `_getTripleCost` 1:1 assumption. `forge build` clean. | `INFO-03`, and supports the `MA-01` / `MA-04` / `MA-05` / `MED-05` dispositions |

The previously-uncommitted `TrustBonding.sol` NatSpec addition (+9 lines, documenting that `totalClaimedRewardsForEpoch`
deliberately measures rewards _claimed_ rather than earned, with the bounding argument) is included in `319303f`. It
changes no code path and closes no finding on its own.

**Four documentation items remain outstanding** and are the cheapest remaining work: `MED-03` (preview warning),
`MED-07` (retune note), `MED-08` (wallet-address coupling), `INFO-01` + `INFO-04` (unit coupling and tier-0 wording).

**Eight code changes and four test items remain** — enumerated at the end of §5b, each requiring its own gating,
mutation-checked regression test with a provenance-free name. None has been started.

**Two reviewer recommendations were formally superseded** rather than carried forward, per the runbook rule, and each is
recorded with its reason in the register: re-deriving `userTier` / `userAvgTier` on redeem (`MA-02` — would enable
salami-sliced withdrawals to walk the average tier down), and adding a dwell requirement (`MA-04` — contradicts a
committed test that asserts activity-driven earning as the intended mechanic). Two more were superseded on design
grounds: an `onlyTimelock` modifier on an `Ownable` curve (`MED-01`) and an account parameter on
`MultiVault.previewRedeem` (`MED-03`).
...
**Bottom line: the gate does not clear yet, but the reason is narrower than the raw severity counts suggest.** Triage
resolved 24 of 35 findings as closed, including three of the five Majors. **The dynamic-fee curve is not deployed or
registered on Intuition Mainnet** (verified, chain `1155`: registry holds 2 curves, `defaultCurveId == 1`), so nothing
in this round is live and no finding is an incident. What remains is a bounded set of pre-ship changes to a contract
that has not shipped.

**Blocking — must be closed before the dynamic-fee curve ships.**

1. **`MA-02(a)` — the minimum-eligible-stake floor.** The single highest-value change in the round, because one fix
   bounds the dust-capture magnitude in `MA-02`, `MA-03` **and** `MA-04` at once. Sub-threshold stake must neither earn
   from a distribution nor dilute a recipient denominator. `generalConfig.feeThreshold` (`1e18`) is the right order of
   magnitude; use a dedicated curve-config field rather than reading MultiVault's, whose semantics differ. Needs a
   gating, mutation-checked test.
2. **`MA-02(b)` — the modeling question, which blocks nothing else but must be answered.** Confirm with the economic
   modeling work whether **stake-independent between-tier allocation** was deliberate in the model the curve was ported
   from. If it was, record it as `Closed — By design` with the floor as the mitigation; if it was not,
   `_weighPriorTiers` needs the stake-scaled weight. Do not change the kernel before this is answered.
3. **`MA-05` + `MED-01` — the fee-cap ceiling and the deploy-script owner.** Close together. The deploy script must pass
   the parameters `TimelockController` rather than `msg.sender`, and `withdrawalCapBps` needs an immutable ceiling so no
   configuration can store a schedule capable of consuming a redemption. The `MultiVault` payout floor is accepted as
   defense in depth, not as the required fix.
4. **`MA-01` — pin the default curve.** Not live and not intended to change, but the constraint is currently enforced by
   nothing but convention. Harden `setBondingCurveConfig` to pin `defaultCurveId` or reject a hook-advertising target.
   The NatSpec that actively asserted the opposite is already corrected (`319303f`).
5. **`MIN-02` — the missing fee-parity gating test**, extended to cover the withdrawal-side front-run mirror. Mutation-
   proven: the load-bearing property of the whole C2 hook surface can be removed with 28 of 28 tests still green. Every
   fix above needs a test that actually goes red.
6. **`MIN-06` — override the inherited `executeBatch(Call[])`.** An auth path with weaker guards than its sibling: the
   solady version applies a different owner notion and no reentrancy guard.

**Pre-upgrade gates — point-in-time facts that must be re-verified immediately before executing the upgrade.** Each
rests on an on-chain value rather than on code, which is exactly why it is a recurring gate and not a one-time check:

7. **AtomWallet census (round-1 `MA-01` / `INFO-08`).** Confirm no wallet has been claimed under the legacy
   `Ownable2Step` model in the interim. No code-level migration was built, so this fact is load-bearing. Converting it
   into an on-chain pre-flight assertion that reverts if any deployed wallet has
   `isClaimed == true && _claimant == address(0)` is cheap and removes a manual step.
8. **Reinitializer version check.** Confirm `_initialized == 1` on the MultiVault proxy before running `reinitialize(2)`
   (verified `1` at time of writing, chain `1155`). Same step, added requirement: **warm the upgrade epoch with any
   MultiVault activity before `upgradeAndCall`** and run the reinitializers in the same operational window (`MED-06`).
9. **`AtomWarden.claimWindow` (`MIN-07`).** The creator-expiry fallback does not exist on the deployed implementation
   (`claimWindow()` reverts, chain `1155`) and ships with this upgrade. Set it deliberately — the intended value is on
   the order of a year, and `0` disables the path entirely. Do not let it default.

**Also outstanding, non-blocking.** Four documentation items (`MED-03`, `MED-07`, `MED-08`, `INFO-01`/`INFO-04`); three
test items (`MIN-03`, `MIN-04`, plus the `MIN-12` CI grep); `MIN-01`'s view split; the unresolved `MED-04` clamp, where
this report and the protocol team disagree and the disagreement is recorded rather than resolved; and the open
`R2-CYF-44` sub-question on whether `TrustBonding` consumes a negative utilization value safely.

**`MIN-04` deserves separate mention** because it is the one item that partially reopens a round-1 disposition: the
storage-layout suite does not pin intra-struct field order, mutation-proved green/green. The fix must use **raw-slot
literal anchors** — a getter-relative assertion cannot detect this class in principle.

**Closed in this pass and no longer gating:** `MIN-12` (provenance markers stripped, `b9510ab`) and `INFO-03` (the
NatSpec that described the wrong failure mode, `319303f`), plus the eight `By design` and twelve `Acknowledged`
dispositions recorded in §5b.
